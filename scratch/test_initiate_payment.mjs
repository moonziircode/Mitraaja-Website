...
async function main() {
  ...
  // Let's use the task code we created in the previous test: MAA-2026060033191119
  // (or create a new one to be fresh, but let's test checking status of the existing one first)
  const taskCode = 'MAA-2026060033191119';
  const transactionNumber = 'TMAA-2026060000179767';

  console.log('\n=== Checking payment status using order/payment/check ===');
  
  // Test 1: bookingId = taskCode
  console.log('\n--- Test 1: bookingId = task_code ---');
  const checkPayload1 = [{ bookingId: taskCode }];
  const res1 = await fetch(`${API_BASE}/order/payment/check`, {
    method: 'POST',
    headers: makeHeaders(token),
    body: JSON.stringify(checkPayload1)
  });
  console.log('Status 1:', res1.status);
  console.log('Response 1:', JSON.stringify(await res1.json(), null, 2));

  // Test 2: bookingId = transaction_number
  console.log('\n--- Test 2: bookingId = transaction_number ---');
  const checkPayload2 = [{ bookingId: transactionNumber }];
  const res2 = await fetch(`${API_BASE}/order/payment/check`, {
    method: 'POST',
    headers: makeHeaders(token),
    body: JSON.stringify(checkPayload2)
  });
