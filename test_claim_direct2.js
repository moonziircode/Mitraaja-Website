...
async function run() {
  ...
  if (tasksData.status === 0 && tasksData.content && tasksData.content.length > 0) {
      const awb = tasksData.content[0].waybill;
      const orderSource = tasksData.content[0].order_source || 'B2B';
      console.log("Found AWB to claim:", awb, "with source:", orderSource);
      
      const claimPayload = {
          agentId: agentStaffId,
          orders: [
              {
      ...
      console.log("Sending Payload:", JSON.stringify(claimPayload, null, 2));
      
      const claimUrl = `${gatewayUrl}/maa-task/order/v2/claim/${encodeURIComponent(awb)}`;
      ...
      console.log("Sending Payload to dummy AWB:", JSON.stringify(claimPayload, null, 2));
      const claimUrl = `${gatewayUrl}/maa-task/order/v2/claim/${encodeURIComponent(awb)}`;
      const claimRes = await fetch(claimUrl, {
          method: 'POST',
          headers: taskHeaders,
      ...
      const claimResult = await claimRes.json();
      console.log("Claim Result (Dummy):", claimResult);
  }
}
