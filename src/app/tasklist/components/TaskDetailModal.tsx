...
import { X, MapPin, Package, Clock, User, Phone, Box, FileText, CreditCard, Loader2 } from "lucide-react";
...
export default function TaskDetailModal({ isOpen, onClose, tasklist, activeTab }: TaskDetailModalProps) {
  ...
  useEffect(() => {
    if (isOpen && code !== "-") {
      ...
      setLiveData(null); // Reset when closed
    }
  }, [isOpen, code]);

  if (!tasklist) return null;
  
  const senderName = liveData?.sender && liveData.sender !== '-' ? liveData.sender : (firstTask.shipperInfo?.name || firstTask.shipper_info?.name || tasklist.owner_name || "-");
  ...
  const recipientName = liveData?.receiver && liveData.receiver !== '-' ? liveData.receiver : (firstTask.recipientInfo?.name || firstTask.recipient_info?.name || "-");
  const recipientPhone = firstTask.recipientInfo?.phone || firstTask.recipient_info?.phone || "-";
  const recipientAddress = liveData?.destination && liveData.destination !== '-' ? liveData.destination : (firstTask.recipientInfo?.address || firstTask.recipient_info?.address || "-");

  const totalWeight = tasklist.tasks?.reduce((acc, t: any) => acc + (t.parcel_total_weight || t.parcelTotalWeight || 0), 0) || 0;
  const serviceType = liveData?.service && liveData.service !== '-' ? liveData.service : (firstTask.product_code || firstTask.productCode || "SD");
  ...
  const statusStr = liveData?.history?.[0]?.status || firstTask.task_status || firstTask.taskStatus || (isTertunda ? "MENUNGGU PICKUP" : "NEW");
          ...
          >
            <div className="sticky top-0 bg-white/80 backdrop-blur-md z-10 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2">
                ...
                {isLoadingLive && <Loader2 className="w-4 h-4 animate-spin text-pink-500" />}
              </h2>
              <button 
                onClick={onClose}
              ...
              <div className="mb-6">
                ...
                <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100">
                  <div className="flex justify-between items-center pb-3 border-b border-gray-200 mb-3">
                    ...
                    <span className="font-semibold text-gray-900">{liveData?.weight && liveData.weight !== '-' ? liveData.weight : `${totalWeight.toFixed(2)} kg`}</span>
                  </div>
                  <div className="flex justify-between items-center pb-3 border-b border-gray-200 mb-3">
