  ...
  const fetchAwbData = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const parsedData: LabelData = {
        ...
        orderSource: task.orderSource || 'Mitraaja',
        ...
        bookingId: task.sourceOrderNo || task.booking_id || task.waybill,
        routingCode: task.routingCode || task.district_code || task.receiverInfo?.districtCode || '0001',
        notes: task.note,
        shipper: {
          name: task.shipperInfo?.name || task.shipperName || '-',
          address: task.shipperInfo?.address || '-',
          phone: task.shipperInfo?.phone || '-',
          city: task.shipperInfo?.city_name || task.shipperInfo?.city || '-',
          zip: task.shipperInfo?.zip || task.shipperInfo?.postcode || '',
          district: task.shipperInfo?.district || task.shipperInfo?.district_name,
          province: task.shipperInfo?.province || task.shipperInfo?.provice_name,
        },
        receiver: {
          ...
          city: task.receiverInfo?.city_name || task.receiverInfo?.city || '-',
          zip: task.receiverInfo?.zip || task.receiverInfo?.postcode || '',
          district: task.receiverInfo?.district || task.receiverInfo?.district_name,
          province: task.receiverInfo?.province || task.receiverInfo?.provice_name,
        },
        items: task.items || [],
      };
