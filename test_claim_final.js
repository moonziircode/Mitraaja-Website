...
async function run() {
  ...
  let orderSource = 'B2B';
  let sourceOrderNo = awb; // fallback
  if (tasksData.status === 0 && tasksData.content && tasksData.content.length > 0) {
      ...
      sourceOrderNo = tasksData.content[0].source_order_no || awb;
  ...
  // NOTE: Android MAA passes `sourceOrderNo` to the `claimKey` field!
  // See line 354754: new MaaClaimTaskRequest(orderSource, sourceOrderNo)
  // But wait, the web app was passing `waybill` (AWB) to `claim_key`!
  // Is this the discrepancy? Let's test with waybill.
  ...
  console.log("Sending payload:", claimPayload);

  try {
      const claimRes = await fetch(`${gatewayUrl}/maa-task/order/v2/claim/${encodeURIComponent(awb)}`, {
          method: 'POST',
          headers: taskHeaders,
          body: JSON.stringify(claimPayload)
      });
      const text = await claimRes.text();
      console.log("Claim Result Status:", claimRes.status);
      console.log("Claim Result Body:", text);
  } catch (err) {
      console.error(err);
  }
}
